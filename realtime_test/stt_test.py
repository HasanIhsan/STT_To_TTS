...
import sys
import os
import threading  # added for thread checks
...
class SimpleApp(QWidget):
    ...
    def audio_stream_stop(self):
        # Guard against joining the play thread from within itself
        if hasattr(self.stream, 'play_thread') and threading.current_thread() is self.stream.play_thread:
            # Already in play thread: only stop playback, don't join
            self.stream.running = False
        else:
            # Called from outside play thread: safe to stop fully
            self.stream.stop()

        # Stop STT recorder and emit final text
        self.recorder.stop()
